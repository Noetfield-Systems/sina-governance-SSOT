...
0k. **decision capacity (LOCKED)** — read `SG-Canonical-Library/noetfield-library/P0-FOUNDATION-SPINE/NF_DECISION_CAPACITY_LOCKED_v1.md` and `data/nf_decision_capacity_v1_LOCKED.json` before inventing next-step chats after Human Tax; path is MISSING_DECISION_CAPACITY → proposal → policy candidate → Motor Learning Organ replay
...
0k. **executive control plane v0 (LOCKED)** — read `SG-Canonical-Library/noetfield-library/P0-FOUNDATION-SPINE/NF_EXECUTIVE_CONTROL_PLANE_V0_LOCKED_v1.md` and `data/nf_executive_control_plane_v0_LOCKED.json` before inventing Super-Agents or LLM decision paths; Next Action is compiled; SourceA `packages/executive-control-plane-v0`; no Cloudflare wire in v0
0l. **executive mesh v1 (LOCKED)** — read `SG-Canonical-Library/noetfield-library/P0-FOUNDATION-SPINE/NF_EXECUTIVE_MESH_V1_LOCKED_v1.md` and `data/nf_executive_mesh_v1_LOCKED.json` before Role Pod / Governor Worker changes; Models propose; Governor commits; reuse Runway Goal Kernel for webpage-repair; no second Railway executor
