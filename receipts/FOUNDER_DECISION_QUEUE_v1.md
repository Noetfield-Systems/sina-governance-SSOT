...
| # | Gate | Act | Unlocks |
|---|---|---|---|
| 21 | **DeepSeek credential dead** — staging worker's `DEEPSEEK_API_KEY` is rejected by the provider (401/402 class); the vault has no DeepSeek key at all. Every blueprint/research job blocks at 0 steps. | Get a valid DeepSeek API key (or top up the account) → replace the `DEEPSEEK_API_KEY` GitHub Actions secret on NOETFIELD-RUNWAY → add it to `~/.sina/secrets.env` | ALL solution-blueprint / research recipe jobs; the Governed Deliverable Proof goes green end-to-end |
| 22 | **Ship the evidence fix to the staging worker** (code is on main, fence needs a PR-shaped check chain) | Merge [PR #197](https://github.com/Noetfield-Systems/NOETFIELD-RUNWAY/pull/197) → run workflow `runway-motor-staging-deploy.yml` with `candidate_sha=<new main sha>`, `confirm=DEPLOY_STAGING_ONLY` → re-run `decision-brief-deliverable-proof.yml` | Telegram receipts name the exact failing rung/provider error instead of "Steps: 0 · $0.000000" — even while gate 21 stays open |

## Added 2026-07-23 (W5-11 revenue receipt adapter — client-money wire)

| # | Gate | Act | Unlocks |
|---|---|---|---|
| 23 | **W5-11→W5-13 — Stripe webhook + dispatch token** (wire is built, proven by test-mode dry-run, and dormant by design; these two are founder account actions no agent may perform) | (a) In the Stripe dashboard, register the webhook endpoint `https://sourceb.ca/api/webhooks/stripe` for event `checkout.session.completed` and put its signing secret on the sourceb.ca worker; (b) set `GOVERNANCE_DISPATCH_TOKEN` (a GitHub token that can fire `repository_dispatch` on sina-governance-SSOT) on the live sourceb.ca worker; (c) deploy the ca surface so main 93eccc5 runs in production | A real paid checkout on sourceb.ca automatically lands a FIRST_REVENUE_RECEIPT (offer: Never Miss a Call) in SG receipts/revenue/ at status FOUNDER_VERIFY — the W5-13 first-dollar countersign is the only remaining human step |
