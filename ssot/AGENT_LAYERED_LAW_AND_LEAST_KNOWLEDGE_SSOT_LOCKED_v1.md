...
- Authority comes from **SG registry ACTIVE status** — not from a library file merely existing.
- SG is guard/authority registry/intake controller; library is asset/law surface repository only.
- **Do not** bulk-promote library files. **Do not** inject library law into agent prompts.
