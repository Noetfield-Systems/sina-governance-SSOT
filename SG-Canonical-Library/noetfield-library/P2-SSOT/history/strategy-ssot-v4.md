# STRATEGY SSOT v4
*Canonical decision frame. Five doctrines, one brain. Reference; do not re-derive.*

> v2 added II (Procedure Over Truth) + III (The Terrain Is Local).
> v3 added IV (Independent Receipt Authority): no agent certifies its own work.
> v4 adds V (Operating Triggers): the named loop and the separation rules that apply
> Doctrine IV to the human↔Claude loop itself.
...
# DOCTRINE V — OPERATING TRIGGERS
*(the loop, and the separation rules that bind it)*

## 0. Core
Doctrine IV (author ≠ subject) applies to the **human↔Claude loop too**, not just the
machine's. These are the named triggers Sina fires, in order. The first is always CHECK —
every loop that broke started from a *report* instead of from reality.

## 1. The six triggers (+ one to close)
1. **CHECK** — external reality only. Pull the live surface cache-busted / read the raw
   artifact; report what *is*. No fix, no spin. Highest truth layer. **Always first.**
2. **ANALYZE** — decompose and price under the doctrines (terrain, EV, trade test). Output =
   decision math + options. No action.
3. **CRITIQUE** — adversary pass; find the hole, force divergence. Aim at Claude's own
   ANALYZE, at an agent report, or at a pasted "CRITIC SAID…". Fire *especially* when voices
   agree — three convergences is the warning light, not the green.
4. **SPEC** — Claude writes the scoped, gated agent prompt (lane / permitted / gated /
   forbidden / stop-state). This is Claude's only "act": authors the order, never touches
   repo or deploy.
5. **VERIFY** — independent external probe of a claim already SUBMITTED *and deployed*.
   Author ≠ subject: Claude's probe is **one** vantage point, paired with Sina's terminal
   curl, never sole authority.
6. **DECIDE** — Claude lays out the gated call (money / legal / deploy / canonical price),
   fully priced. **Sina pulls the trigger. Never Claude.**

**0. STOP** — clean stop-state, no new action, no hook. Closes a session.
...
## 3. Separation rules (Doctrine IV, applied to this loop)
- If Claude **SPEC**s a fix, Claude's **VERIFY** of that same fix is **advisory only** —
  Sina's independent curl is the authority. Claude cannot grade the outcome of its own order.
- **CRITIQUE Claude's ANALYZE before SPEC.** Never ship self-graded reasoning.
- **DECIDE is always Sina's.** Claude prices it; Claude never pulls it.

## 4. Recovery rule
Mid-thread and unsure which trigger you're in? You're almost always **one step earlier than
you think.** Back up to CHECK or ANALYZE before SPEC. New thread always opens on CHECK.
