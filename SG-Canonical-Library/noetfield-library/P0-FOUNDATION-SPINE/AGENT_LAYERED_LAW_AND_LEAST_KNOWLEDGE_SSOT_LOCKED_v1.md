...
7. **Use canonical words from `NOETFIELD_TERMINOLOGY_v1` on every output** (wording authority). Shorthand, synonyms, and model habit lose to terminology lint. Meaning disputes and **any new job, task, specialist, role, page, clause, or receipt field** require a **dictionary entry first** — terminology is minted from dictionary, never the reverse.
