...
Q1=$(python3 scripts/query_repo_graph_v1.py __VERIFY_SUBSYSTEM__ 2>&1)
Q1_RC=$?
if [ "$Q1_RC" = "0" ] && echo "$Q1" | grep -q "^# subsystem: __VERIFY_SUBSYSTEM__"; then
  check "query_subsystem_works" "0" "query_repo_graph_v1.py __VERIFY_SUBSYSTEM__ returned a subsystem listing"
else
  check "query_subsystem_works" "1" "query_repo_graph_v1.py __VERIFY_SUBSYSTEM__ did not return expected output (exit=$Q1_RC)"
fi

# 4b. query by keyword works
Q2=$(python3 scripts/query_repo_graph_v1.py __VERIFY_KEYWORD__ 2>&1)
Q2_RC=$?
if [ "$Q2_RC" = "0" ] && echo "$Q2" | grep -q "__VERIFY_KEYWORD__"; then
  check "query_keyword_works" "0" "query_repo_graph_v1.py __VERIFY_KEYWORD__ returned matches"
else
  check "query_keyword_works" "1" "query_repo_graph_v1.py __VERIFY_KEYWORD__ did not return expected output (exit=$Q2_RC)"
fi
