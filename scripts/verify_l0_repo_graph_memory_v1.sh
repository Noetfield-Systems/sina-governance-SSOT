...
Q2=$(python3 scripts/query_repo_graph_v1.py README 2>&1)
Q2_RC=$?
if [ "$Q2_RC" = "0" ] && echo "$Q2" | grep -q "README"; then
  check "query_keyword_works" "0" "query_repo_graph_v1.py README returned matches"
else
  check "query_keyword_works" "1" "query_repo_graph_v1.py README did not return expected output (exit=$Q2_RC)"
fi
